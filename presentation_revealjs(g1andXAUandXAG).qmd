...
* **Hedge Ratio Dynamics ($B$):** We tested both fixed beta and **Dynamic Beta (1.5 day)**. The rolling covariance/variance ratio was chosen to capture the evolving structural relationship between the two indices.
* **Mean Reversion Memory ($W$):** We evaluated lookback windows for the Z-Score from **30min to 60min**. A **shorter 45min window** was finally selected to respond rapidly to mean-reversion opportunities in 1-minute frequency data.
...
* **Momentum Window ($M$):** We tested various windows from intra-day to multi-week. We selected **1 trading day** to capture immediate price shifts in the market.
* **Rebalance Frequency ($R$):** We matched the rebalance cycle to the momentum window of **1 trading day**. This ensures the portfolio is updated daily to reflect the most recent relative strength between assets.
